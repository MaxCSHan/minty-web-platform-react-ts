...
const NewRoom = () => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Backspace') {
      setSelectedUser(selectedUser.slice(0, selectedUser.length - 2))
    }
  }
    ...
    <div className=" overflow-hidden h-screen sm:h-auto  w-screen sm:w-96 md:w-120 lg:w-160 bg-white   sm:border flex flex-col">
      <div className="fixed z-20 sm:static h-12 sm:h-16 bg-white w-full flex  items-center justify-center px-2 sm:px-8 border-b">
        ...
        <div className="h-16  w-full flex items-center justify-center">
          ...
          <div
            className={`font-bold text-lg ${selectedUser.length > 0 ? 'text-black  cursor-pointer' : 'text-gray-400 cursor-default'}`}
      ...
      {/* <div
        className={`${
          selectedUser.length > 0 ? 'visible opacity-100 h-24' : 'invisible opacity-0 h-0'
