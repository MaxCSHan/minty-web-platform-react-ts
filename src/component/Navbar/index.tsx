...
const MiniSearchbar = () => {
  ...
  return (
    <div
      className={`relative ml-1 sm:ml-3 h-8 sm:h-10 w-60 sm:w-160 z-20  border rounded-full flex items-center ${
        inputValue && isFocus ? 'border-b-0' : ''
      }`}
    >
      ...
      <input
        className={`appearance-none w-full h-8 sm:h-10 z-20  border-t rounded-r-full bg-gray-100 focus:bg-white outline-none transition duration-200 ease-in-out ${
          inputValue && isFocus ? 'border-b-0' : ' border-b'
        }`}
        type="text"
        value={inputValue}
      ...
      >
        <div className="h-full flex flex-col pt-8 ">
          {recommendations.map((ele, index) => (
            <div className="h-10 pl-10  flex items-center hover:bg-gray-50 " key={`recommendation_${index}`} onClick={() => clickSearch(ele)}>
              {ele}
            </div>
          ))}
          ...
          {Skills.map((skill, index) => (
            <div
              className="h-10 pl-10  flex items-center hover:bg-gray-50 last:rounded-b-3xl font-bold"
...
const Navbar = () => {
  ...
  return (
    <div
      className={`fixed h-14  w-screen z-20 bg-white dark:bg-gray-800 dark:text-white items-center border-b justify-between ${
        location === '/search' ? ' shadow-none' : 'shadow-lg'
      } ${location.includes('/chat/room') ? 'hidden sm:flex' : 'flex'}`}
    >
      ...
      <div className="h-full w-48 md:w-64  flex items-center justify-around ">
        ...
        <div className="relative z-40">
          <button className="focus:outline-none" onClick={() => setIsDropdown(!isDropdown)} onBlur={() => setIsDropdown(false)}>
          ...
          <div
            className={`origin-top-right absolute  -right-10 mt-2 py-2 w-80 sm:w-120 h-140 z-40 text-xs sm:text-base rounded-md bg-white border  focus:outline-none ${
              isDropdown ? 'opacity-100 scale-100 visible ' : 'opacity-0 scale-90 invisible '
          ...
          >
            {notifications.map((note, index) => (
              <div className="h-12 border-b border-gray-50 flex items-center px-2  z-40" key={`notification_${index}`}>
        ...
        <Link to={`/User/${loginUser()?.uid}`}>
          <div className="cursor-pointer">
            {loginUser() ? (
              <img className="rounded-full h-10 w-10" alt="userProfile" src={loginUser().avatar}></img>
            ) : (
